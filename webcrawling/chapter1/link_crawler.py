import re
from urllib import robotparser,parse,request
import urllib
import time
...
def normalize(seed_url, link):
    ...
    """
    link, _ = parse.urlparse.urldefrag(link)  # remove hash to avoid duplicates
    return parse.urlparse.urljoin(seed_url, link)


def same_domain(url1, url2):
    """Return True if both URL's belong to same domain
    """
    return parse.urlparse.urlparse(url1).netloc == parse.urlparse.urlparse(url2).netloc
